...
#ifndef DUNE_CONTROL_PATH_FORMATION_CONTROLLER_HPP_INCLUDED_
#define DUNE_CONTROL_PATH_FORMATION_CONTROLLER_HPP_INCLUDED_
...
namespace DUNE
{
  namespace Control
  {
    ...
    static const float c_f_erry_factor = 1.75f;

    // Export DLL Symbol.
    class DUNE_DLL_SYM PathFormationController;

    class PathFormationController: public Tasks::Task
    {
    public:
      //! Constructor.
      PathFormationController(std::string name, Tasks::Context& ctx);

      //! Destructor.
      virtual
      ~PathFormationController(void);
      ...
      //! Consumer for EstimatedLocalState message.
      //! @param es message to consume.
      void
      consume(const IMC::EstimatedLocalState* es);
      ...
      //! EstimatedLocalState entity to use
      std::string m_state_entity;
      ...
      virtual void
      step(const IMC::EstimatedLocalState& state, const TrackingState& ts) = 0;
      ...
      //! @param ts tracking state information
      virtual void
      loiter(const IMC::EstimatedLocalState& state, const TrackingState& ts);
      ...
      //! Handler for EstimatedLocalState source id filter.
      //! This is called when an EstimatedLocalState is received.
      //! By default it only passes EstimatedLocalState messages from the system itself.
      //! @param[in] es EstimatedLocalState message.
      //! @return true if the message is NOT allowed to pass.
      virtual bool
      sourceFilter(const IMC::EstimatedLocalState* es)
      {
        // Allow only EstimatedLocalState from the same vehicle.
        return es->getSource() != getSystemId();
      }

      //! Convert EstimatedLocalState to EstimatedState
      void
      convertToEstimatedState(IMC::EstimatedState* es, const IMC::EstimatedLocalState* els);
    ...
    private:
      ...
      // Arguments
      //! EstimatedLocalState received and valid
      bool m_els_valid;
      //! DesiredPath received and valid
      bool m_dp_valid;
      //! Control period
      double m_cperiod;
      //! State report period
      ...
      //! Threshold for a jump in EstimatedLocalState to turn monitors off
