...
namespace DUNE
{
  namespace Control
  {
    ...
    PathFormationController::PathFormationController(std::string name, Tasks::Context& ctx):
      Task(name, ctx),
      m_bt_entity(NULL),
      ...
      m_scope_ref(0)
    {
      ...
      .defaultValue("10")
      .description("Control frequency (< 0 for event-driven EstimatedLocalState processing)")
      .units(Units::Hertz);
      ...
      .defaultValue("7.0")
      .units(Units::Meter)
      .description("Threshold for a jump in EstimatedLocalState to turn monitors off");
      ...
      param("EstimatedLocalState Entity Label", m_state_entity)
      .description("Filter ESL on the label specified");
    ...
    PathFormationController::~PathFormationController(void)
    {
      Memory::clear(m_btrack);
    }

    void
    PathFormationController::onUpdateParameters(void)
    {
    ...
    void
    PathFormationController::onResourceInitialization(void)
    {
      requestDeactivation();
    }

    void
    PathFormationController::onResourceRelease(void)
    { }

    void
    PathFormationController::onEntityReservation(void)
    {
    ...
    void
    PathFormationController::consume(const IMC::Brake* brake)
    {
    ...
    void
    PathFormationController::consume(const IMC::DesiredPath* dpath)
    {
      if (!isActive())
      {
        err(DTR("not active"));
      ...
      if (!m_els_valid)
        war("EstimatedLocalState not received yet, must wait for valid message");
      ...
      // Call path startup handler when els valid from consume(els).
      m_dp_valid = true;
    }

    void
    PathFormationController::consume(const IMC::NavigationUncertainty* nu)
    {
      m_ctm.nav_uncertainty = m_ctm.nav_unc_factor * std::sqrt(std::max(nu->x, nu->y));
    }

    void
    PathFormationController::consume(const IMC::Distance* dist)
    {
      if (isTrackingBottom())
    ...
    void
    PathFormationController::consume(const IMC::DesiredZ* zref)
    {
      if (isTrackingBottom())
    ...
    void
    PathFormationController::consume(const IMC::DesiredSpeed* dspeed)
    {
      if (isTrackingBottom())
    ...
    void
    PathFormationController::consume(const IMC::EstimatedLocalState* els)
    {
      // Pass EstimatedLocalStates from the specified vehicle with the specified entity
      if (sourceFilter(els) || resolveEntity(els->getSourceEntity()).c_str() != m_state_entity)
        return;

      // Save new EstimatedLocalState values
      m_elstate  = *els;
      m_estate  = *els->state;
      ...
      if (m_dp_valid)
      {
        m_dp_valid = false; //reset flag in order to only call onPathStartup when DesiredPath is received
        debug("DesiredPath recived, first ELS received as well, start path-ctrl");
        onPathStartup(m_elstate, m_ts);
      }

      IMC::EstimatedState* es = new IMC::EstimatedState();
      es = &m_estate;
      ...
      {
        //war("More than %f s since last state received (diff=%f s)",m_cperiod,now - m_ts.now);
        return;
      }
    ...
    void
    PathFormationController::updateTrackingState(void)
    {
      ...
      if (!m_ts.loitering)
      {
        ...
        if (errx <= erry && erry < c_f_erry_factor * m_time_factor * s)
          m_ts.eta = errx / s;
        else
    ...
    bool
    PathFormationController::navigationJumped(const IMC::EstimatedState* new_state,
    ...
    void
    PathFormationController::monitorAlongTrackError(void)
    {
    ...
    void
    PathFormationController::monitorCrossTrackError(void)
    {
    ...
    void
    PathFormationController::consume(const IMC::ControlLoops* cloops)
    {
    ...
    void
    PathFormationController::onActivation(void)
    {
      m_error = false;
      m_tracking = false;
      m_braking = false;
    ...
    void
    PathFormationController::onDeactivation(void)
    {
      if (m_ts.z_control)
    ...
    void
    PathFormationController::signalError(const std::string& msg)
    {
      m_error = true;
      err("%s", msg.c_str());
    ...
    void
    PathFormationController::updateEntityState(const std::string msg)
    {
      if (m_setup)
    ...
    void
    PathFormationController::configureControlLoops(uint8_t enable, uint32_t mask)
    {
      if (enable)
      {
    ...
    void
    PathFormationController::reportPathControlState(bool force = false)
    {
    ...
    void
    PathFormationController::loiter(const IMC::EstimatedLocalState& state, const TrackingState& ts)
    {
      ...
      lts.track_pos.x = 0;
      lts.los_angle = getBearing(*state.state.get(), lts.end);

      step(state, lts);
    }

    void
    PathFormationController::deactivateBottomTracker(void)
    {
      if (m_btrack == NULL)
        return;
    ...
    void
    PathFormationController::onMain(void)
    {
      while (!stopping())
      {
        waitForMessages(1.0);
