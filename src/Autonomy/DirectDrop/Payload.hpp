...
public:
  ...
  State fallingObjectFunction( State state, EstimatedStreamVelocity wind )
  {
    ...
    state_dot.vx = -0.5*C_D*A*rho*V_a*state.vx;
    state_dot.vy = -0.5*C_D*A*rho*V_a*state.vy;
    state_dot.vz = -g-0.5*C_D*A*rho*V_a*state.vz;
    return state_dot;
  }
  ...
  State RK4_fallingObject( State state_in, EstimatedStreamVelocity wind )
  {
    ...
    return state_in + dt/6*(k1+k2+k3+k4);
  }
  ...
  void set_target( IMC::Target target_in)
  {
    ...
    received_target.z = target_in.z;
  }

  void set_release_state( Point UAV_state )
  {
    release_state = UAV_state;
  }

  void load_target_error( fp32_t target_error_placekeeper[])
  {
    WGS84::displacement(received_target.lat,received_target.lon,received_target.z,
        estimated_hitpoint.lat,estimated_hitpoint.lon,estimated_hitpoint.z,
  ...
  {
    WGS84::displacement(received_target.lat,received_target.lon,received_target.z,
        CARP_circle.centre_lat,CARP_circle.centre_lon,CARP_circle.z,
  ...
  {
    WGS84::displacement(received_target.lat,received_target.lon,received_target.z,
        release_state.lat,release_state.lon,release_state.z,
        &placekeeper[0],&placekeeper[1],&placekeeper[2]);
  }

  void calculate_CARP_circle( EstimatedStreamVelocity wind_in, fp32_t desired_speed, fp32_t drop_altitude)
  {
    CARP_speed = desired_speed;
    printf("CARP_speed: %f\n", CARP_speed);
    CARP_circle.z = drop_altitude + received_target.z;
    ...
    falling_object_with_wind.z = CARP_circle.z;
    falling_object_with_wind.vx = wind_in.x/wind_speed*CARP_speed;
    falling_object_with_wind.vy = wind_in.y/wind_speed*CARP_speed;
    ...
    int counter = 0;
    while( falling_object_with_wind.z > received_target.z && counter < 10000)
    {
      counter ++;
      falling_object_with_wind = RK4_fallingObject( falling_object_with_wind, wind_in );
//      printf("x %f, y %f, z %f, vx %f, vy %f, vz %f\n",falling_object_with_wind.x,falling_object_with_wind.y,falling_object_with_wind.z,falling_object_with_wind.vx,falling_object_with_wind.vy,falling_object_with_wind.vz);
    }
    printf("Counter: %d", counter);
    printf("Falling object: x: %f, y: %f z: %f \n", falling_object_with_wind.x,falling_object_with_wind.y,falling_object_with_wind.z);
//    Point point_with_wind;
//    point_with_wind.lat = received_target.lat;
//    point_with_wind.lon = received_target.lon;
//    point_with_wind.z = desired_height;
//    point_with_wind.vx = wind_in.x/wind_speed*CARP_speed;
//    point_with_wind.vy = wind_in.y/wind_speed*CARP_speed;
//    point_with_wind.vz = 0.0;
//    WGS84::displace(-falling_object_with_wind.x,-falling_object_with_wind.y,&point_with_wind.lat,&point_with_wind.lon);
    ...
    falling_object_against_wind.z = CARP_circle.z;
    falling_object_against_wind.vx = -wind_in.x/wind_speed*CARP_speed;
    falling_object_against_wind.vy = -wind_in.y/wind_speed*CARP_speed;
    falling_object_against_wind.vz = 0.0;
    counter = 0;
    while( falling_object_against_wind.z > received_target.z && counter < 10000)
    {
      counter ++;
      falling_object_against_wind = RK4_fallingObject( falling_object_against_wind, wind_in );
    }
    printf("Counter: %d", counter);
    printf("Falling object: x: %f, y: %f z: %f \n", falling_object_against_wind.x,falling_object_against_wind.y,falling_object_against_wind.z);
//    Point point_against_wind;
//    point_against_wind.lat = received_target.lat;
//    point_against_wind.lon = received_target.lon;
//    point_against_wind.z = desired_height;
//    point_against_wind.vx = wind_in.x/wind_speed*CARP_speed;
//    point_against_wind.vy = wind_in.y/wind_speed*CARP_speed;
//    point_against_wind.vz = 0.0;
//    WGS84::displace(-falling_object_against_wind.x,-falling_object_against_wind.y,&point_against_wind.lat,&point_against_wind.lon);
  ...
  void calculate_estimated_hitpoint( EstimatedStreamVelocity release_wind )
  {
    //Declaring and initializing variables
    State falling_object;
    falling_object.x = 0.0;
    falling_object.y = 0.0;
    falling_object.z = release_state.z;
    ...
    estimated_hitpoint.z = release_state.z;
    estimated_hitpoint.vx = release_state.vx;
    estimated_hitpoint.vy = release_state.vy;
    estimated_hitpoint.vz = release_state.vz;
    printf("Distance from drop position to CARP center: %f\n", WGS84::distance(estimated_hitpoint.lat,estimated_hitpoint.lon,estimated_hitpoint.z,
        CARP_circle.centre_lat,CARP_circle.centre_lon,CARP_circle.z));
    int counter = 0;
    while( falling_object.z > received_target.z && counter < 10000)
    {
      counter++;
    ...
    estimated_hitpoint.z = falling_object.z;
    printf("Falling object: x: %f, y: %f, z: %f. Target z: %f\n\n", falling_object.x,falling_object.y,falling_object.z,received_target.z);
    printf("Distance between estimated hit point and target: %f\n",
        WGS84::distance(received_target.lat,received_target.lon,received_target.z,estimated_hitpoint.lat,estimated_hitpoint.lon,estimated_hitpoint.z));
  }
};
