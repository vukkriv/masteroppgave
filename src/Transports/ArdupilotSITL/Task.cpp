...
namespace Transports
{
  namespace ArdupilotSITL
  {
    ...
    struct Task: public DUNE::Tasks::Task
    {
      ...
      Matrix
      matrixT(float roll, float pitch, float yaw)
      {

        (void) yaw;

        double t_elements[9] = { 1, std::sin(roll) * std::tan(pitch), std::cos(roll) * std::tan(pitch),
                                  0, std::cos(roll), -std::sin(roll),
                                  0, std::sin(roll) / std::cos(pitch), std::cos(roll) / std::cos(pitch) };


        return Matrix(t_elements, 3, 3);
      ...
      consume(const IMC::SimulatedState* simstate)
      {
        ...
        // Convert rotational rates to earth frame..


        Matrix pqr_ned = Matrix(3, 1, 0.0);
        Matrix pqr_body = Matrix(3,1,0.0);

        pqr_body(0) = simstate->p;
        pqr_body(1) = simstate->q;
        pqr_body(2) = simstate->r;

        pqr_ned = matrixT(simstate->phi, simstate->theta, simstate->psi) * pqr_body;


        fdm.rollRate = Math::Angles::degrees(pqr_ned(0));
        fdm.pitchRate = Math::Angles::degrees(pqr_ned(1));
        fdm.yawRate = Math::Angles::degrees(pqr_ned(2));
      ...
      consume(const IMC::PWM* msg)
      {
        ...
        {
          debug(DTR("Got PWM message from unknown entity."));
        }
      }
