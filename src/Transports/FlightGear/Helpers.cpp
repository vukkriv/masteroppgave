...
namespace Transports
{
  namespace FlightGear
  {
    ...
    void
    convertFromEstimatedStateToFGNetHost(const IMC::EstimatedState* estate, IMC::Acceleration* acc, FGNetFDM* net)
    {
      // Set version!
      net->version = FG_NET_FDM_VERSION;

      // Positions
      net->latitude = estate->lat;
      net->longitude = estate->lon;
      net->altitude = estate->height;

      net->agl = 0; //estate->height; // Agl: Above ground level.

      net->phi = estate->phi;
      net->theta = estate->theta;
      net->psi = estate->psi;
      ...
      //fp32_t vx, vy, vz;
      //BodyFixedFrame::toInertialFrame(estate->phi, estate->theta, estate->psi,
      //  estate->u, estate->v, estate->w,
      //  &vx, &vy, &vz);

      net->v_north = estate->u * METER_TO_FEET;
      net->v_east = estate->v * METER_TO_FEET;
      net->v_down = estate->w * METER_TO_FEET;
