...
namespace Transports
{
  namespace FlightGear
  {
    ...
    struct Arguments
    {
      ...
      //! Which state should FG display?
      std::string state_to_display;
    ...
    struct Task : public DUNE::Tasks::Task
    {
      ...
      //! Which state to listen to and forward to fg. (estimated or simulated)
      StateSource m_source_state;
        ...
        DUNE::Tasks::Task(name, ctx)
      {
        ...
        param("State to display", m_args.state_to_display)
        .defaultValue("Simulated State")
        .values("Estimated State,Simulated State")
        .description("Sets which state (Estimated or Simulated State) that should be transported to FG");
      ...
      onUpdateParameters(void)
      {
        ...
        if( m_args.state_to_display == "Estimated State")
          m_source_state = ESTATE;
        else
          m_source_state = SSTATE;
      }
      ...
      consume(const IMC::SimulatedState* simstate)
      {
        ...
        if( m_source_state != SSTATE )
        {
          trace("Discarding simulated state");
          return;
        }
      ...
      {
        trace("Vizualiser got new EstimatedState data.");

        if( m_source_state != SSTATE )
        {
          trace("Discarding estimated state");
          return;
        }
        ...
        // Simulation is done in NED. Calculate to geodetic coordinates.
        fp64_t rcv_lat = estate->lat;
        fp64_t rcv_lon = estate->lon;
        fp64_t rcv_hei = estate->height;

        WGS84::displace(estate->x, estate->y, estate->z,
            &rcv_lat, &rcv_lon, &rcv_hei);

        IMC::EstimatedState state = *estate;

        state.lat = rcv_lat;
        state.lon = rcv_lon;
        state.height = rcv_hei;


        // Prepare for transmission
        FGNetFDM net;
        convertFromEstimatedStateToFGNetHost(&state, &m_acc, &net);
        convertToNetworkEndian(&net);
